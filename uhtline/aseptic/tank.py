...
class AsepticTank:
    ...
    def _load(self) -> None:
        ...
        self._pressure_history = [float(item) for item in stored.payload.get("pressure_history", [])]
    ...
    def sterilize(self, *, confirmation_id: str, reason: str) -> dict[str, Any]:
        """Accept only a confirmation this line issued; a superseded one is refused here."""
        ...
        try:
            ...
            raise StateError(
                "the sterilization confirmation was never issued on this line",
                tank="aseptic",
                supplied=supplied,
            ) from exc
        self._sterile = True
